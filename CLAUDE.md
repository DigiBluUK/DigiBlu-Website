...
- The `pages-only` branch is merged into `dev` (21 Sep 2026) and can be deleted once nobody needs its preview link.
...
- `dev` is ahead of `main`: the Safari Destinations correction and quote, contact-form spacing, pages instead of dialogs, the enquiry event, the standalone pages' nav and scroll-to-top, the compact cookie banner, and the policy lines awaiting approval. Release pending, then the DNS switch.
