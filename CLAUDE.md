...
- Dialogs to pages (Radu): built on the `pages-only` branch (preview `https://pages-only-digiblu-website.radu-ghitescu.workers.dev/`), the contact form included, with the `generate_lead` event and its line in section 12 of the policy; awaiting DigiBlu's decision. `dev` still has the dialogs. If approved, merge `pages-only` into `dev`, then release; if not, delete the branch.
